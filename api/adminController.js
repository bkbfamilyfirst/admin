...
exports.getKeyTransferLogs = async (req, res) => {
    try {
        ...
        if (distributorId) matchStage.toUser = new mongoose.Types.ObjectId(distributorId); // Assuming distributorId is an ObjectId
        ...
        let pipeline = [
            ...
            },
            {
                $unwind: {
                    path: '$fromUser',
                    preserveNullAndEmptyArrays: true // Keep logs even if fromUser is null
                }
            },
            {
                $lookup: {
            ...
            },
            {
                $unwind: {
                    path: '$toUser',
                    preserveNullAndEmptyArrays: true // Keep logs even if toUser is null
                }
            }
        ];
        ...
        pipeline.push(
            { $sort: { date: -1 } }, // Assuming 'timestamp' in frontend maps to 'date' in backend
            {
                $facet: {
        ...
        const [result] = await KeyTransferLog.aggregate(pipeline);
